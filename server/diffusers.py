"""Stable Diffusion image generation (diffusers, 100% lokal).

Model ditaruh manual di server/storage/diffusers/<model_id>/ — Waves
tidak pernah mengunduh bobot: semua load memakai local_files_only=True.
Dua layout folder didukung (lihat README di folder itu):

  A) Folder pipeline diffusers lengkap (model_index.json + unet/vae/...).
  B) Satu file checkpoint (*.safetensors / *.ckpt) dipakai via
     from_single_file(); config.json opsional bisa menyebut `vae` lokal.
"""
...
# Pipeline dimuat sekali lalu dipakai bersama, di-cache per (model, device).
...
def _pretty_name(model_id: str) -> str:
    """Ubah id folder jadi nama yang rapi, mis. 'tiny-sd' -> 'Tiny SD'."""
...
def list_models() -> list[dict]:
    """Scan folder sd/ dan tampilkan tiap folder yang punya model (mode A
    atau B). Tanpa diffusers yang terpasang, daftar tetap bisa ditampilkan."""
...
def _img2img_pipe(pipe: object, device: str = "cpu"):
    """Bangun img2img pipeline yang berbagi komponen yang sudah dimuat,
    supaya tidak memuat model berbeda dua kali (hemat VRAM/RAM)."""
    import torch
...
def generate(
    ...
    """Jalankan pipeline dan kembalikan (list PNG bytes, seed terpakai)."""
    import torch
...
class GenerateRequest(BaseModel):
    ...
    init_image_b64: str | None = Field(
        None,
        description="Data URL atau base64 PNG untuk image-to-image. "
        "Kalau diisi, ukuran output mengikuti gambar ini.",
    )
...
def generate_image(req: GenerateRequest):
    # Plain `def` (bukan async) supaya FastAPI menjalankan inferensi yang
    # berat ini di threadpool, tidak memblokir event loop — sama seperti
