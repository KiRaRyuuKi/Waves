...
taruh isinya di `server/storage/diffusers/<model_id>/`:

```bash
# di komputer dengan internet (TIDAK dijalankan oleh Waves)
huggingface-cli download runwayml/stable-diffusion-v1-5 \
  --local-dir server/storage/diffusers/sd-v1-5
```
...
`server/storage/diffusers/<model_id>/`, plus `config.json` opsional:
