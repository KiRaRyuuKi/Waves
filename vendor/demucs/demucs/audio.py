...
def save_audio(wav: torch.Tensor,
    ...
    if suffix == ".mp3":
        ...
        # Written via `soundfile` (libsndfile) rather than torchaudio.save,
        # which on recent torchaudio releases requires the separate
        # `torchcodec` package (and a matching FFmpeg/PyTorch ABI) just to
        # write a WAV file. soundfile has no such dependency.
        if as_float:
            subtype = "FLOAT"
        else:
            subtype = {16: "PCM_16", 24: "PCM_24", 32: "PCM_32"}[bits_per_sample]
        data = wav.data.cpu().numpy().T  # soundfile wants (frames, channels)
